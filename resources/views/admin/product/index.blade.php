        ...
        <div class="col-12">
            <div class="card mb-4">
                <div class="d-flex justify-content-between card-header pb-0">
                    <h1 class="fs-4 fw-bold">Product List</h1>
                        ...
                        <table class="table align-items-center mb-0">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center pe-2 text-uppercase text-secondary text-xs font-weight-bolder opacity-9 ps-2">Category</th>
                                    <th class="text-center pe-2 text-uppercase text-secondary text-xs font-weight-bolder opacity-9 ps-2">Image</th>
                                    <th class="text-center pe-2 text-uppercase text-secondary text-xs font-weight-bolder opacity-9 ps-2">Quantity</th>
                                    <th class="text-center pe-2 text-uppercase text-secondary text-xs font-weight-bolder opacity-9 ps-2">Selling_price</th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="align-middle text-center">
                                        @if($product->image)
                                        <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->name }}" class="img-fluid" style="max-height: 7rem;">
                                        @else
                                        <img src="https://source.unsplash.com/500x500?{{ $product->name }}" class="img-fluid" alt="{{ $product->name }}" style="max-height: 7rem;">
                                        @endif
                                    </td>

                                    <td class="align-middle text-center">{{ $product->qty }}</td>
                                    <td class="align-middle text-center">{{ $product->selling_price }}</td>

                                    <td class="align-middle text-center">
                                        <a href="/dashboard/products/{{ $product->id }}" class="badge bg-info font-weight-bold text-xs me-1" data-toggle="tooltip" data-original-title="Edit user"><i class="bi bi-eye"></i>
                                        </a>

                                        <a href="/dashboard/products/{{ $product->id }}/edit" class="badge bg-warning font-weight-bold text-xs me-1" data-toggle="tooltip" data-original-title="Edit user"><i class="bi bi-pencil-square"></i>
                                        </a>
